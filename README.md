...
Изучить основы параллельного программирования с использованием MPI, реализовать параллельный алгоритм умножения квадратных матриц, провести вычислительные эксперименты и оценить производительность.
...
В MPI используется модель распределённой памяти, где данные и вычисления делятся между процессами.

---

## 3. Описание алгоритма

1. Инициализация MPI (`MPI_Init`)
2. Определение числа процессов и ранга
3. Разбиение строк матрицы между процессами
4. Каждый процесс вычисляет свою часть результата
5. Используется барьер синхронизации (`MPI_Barrier`)
6. Замер времени через `std::chrono`
7. Повтор эксперимента 3 раза
8. Сохранение результатов в CSV
9. Формирование итогового отчёта
...
```bash
mpicxx lab5.cpp -o lab5
```

Запуск (обязательно 4 процесса):
...
* `experiment_results.csv` — подробные результаты
...
| ------ | --------- | ---------- | ---------- |
| 200    | 0.0836    | 0.0425     | 0.0217     |
| 400    | 0.6574    | 0.3293     | 0.1663     |
| 800    | 4.8688    | 2.5543     | 1.3306     |
| 1200   | 15.905    | 8.0841     | 4.2759     |
| 1600   | 37.918    | 19.0462    | 9.8496     |
| 2000   | 73.732    | 36.9853    | 19.0133    |
...
* Ускорение близко к линейному
* Эффективность выше для больших матриц
* Накладные расходы заметны на малых размерах
* Распределение строк обеспечивает равномерную нагрузку
  
  Время в зависимости от процесса
<img width="601" height="349" alt="image" src="https://github.com/user-attachments/assets/c3c0981f-eee7-4ac6-9852-3425088ae0d1" />

Ускорение
<img width="608" height="384" alt="image" src="https://github.com/user-attachments/assets/7f44f3ea-7fc9-4cb4-9f4f-e297a0f31922" />
...
* Реализован параллельный алгоритм умножения матриц с использованием MPI
* Проведены эксперименты для различных размеров матриц
* Получено ускорение до ~4 раз на 4 процессах

Основные результаты:

* Подтверждена сложность O(N³)
* Достигнута высокая эффективность параллелизации
* MPI показал хорошую масштабируемость
